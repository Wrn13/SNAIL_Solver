...
Batch driver for ``zhou_coupler.ZhouCoupler``, the dressed-mode (first-principles)
model. Rather than driving an adiabatically-eliminated effective model, it builds
...
from snail_solver.sweep_common import (
    _drag_skip_GHz, _nearest_collision, _stark_offset_GHz,
...
def main() -> None:
    ...
    sub.add_argument("--drag-subharmonic", action="store_true",
                     help="let DRAG target the nearest SUBHARMONIC collision (the pump's "
                          ...
                          "ON BY DEFAULT -- this flag now only pins the default explicitly")
    sub.add_argument("--no-drag-subharmonic", action="store_true",
                     help="turn the (default-on) subharmonic DRAG channels OFF, to "
                          "reproduce a run from before they became the default. Unlike "
                          "--drag-subharmonic this also overrides a value baked into an "
                          "existing grid.json at prepare time")
    ap.add_argument("--subharmonic-modes", default=None,
                    help="comma list selecting which subharmonic channels to include: "
                         "subset of {a,b,spec,s}. e.g. 'spec' isolates the swept spectator "
                         "subharmonic; 's' the SNAIL/coupler one. Default all. Pass an "
                         "empty string to select NONE (same as --no-drag-subharmonic).")
    ...
    if args.subharmonic_modes is not None:
        # `is not None`, so an EMPTY --subharmonic-modes really selects none: "".split(",")
        # filters to [], and _collision_candidates now honours an empty list literally.
    ...
    if args.drag_subharmonic:
        config["drag_subharmonic"] = True
    if args.no_drag_subharmonic:
        # The one flag here that can force a value OFF. Needed because subharmonic DRAG is
        # now the default, so a grid prepared today bakes in True and there would otherwise
        # be no way to re-run it without the subharmonic channels.
        config["drag_subharmonic"] = False
    if args.subharmonic_modes is not None:
        config["subharmonic_modes"] = [m.strip() for m in
                                       args.subharmonic_modes.split(",") if m.strip()]
